...
async function promptShareContact(chatId: number): Promise<void> {
  await sendMessage(
    chatId,
    'Нажмите кнопку ниже, чтобы поделиться номером телефона. Это нужно для входа на Vafo.tj и уведомлений по сохранённым квартирам.\n\n⚠️ Не печатайте номер вручную — Telegram не сможет проверить, что он ваш. Используйте кнопку.',
    {
      replyMarkup: {
        keyboard: [
...
async function handleStartCommand(message: TgMessage): Promise<void> {
  ...
  if (!token) {
    await sendMessage(
      chatId,
      'Привет! Чтобы войти на Vafo.tj, откройте сайт и нажмите «Войти через Telegram» — затем вернитесь сюда по ссылке.',
    );
    return;
  }
  ...
  if (
    ...
    await sendMessage(
      chatId,
      'Ссылка устарела. Откройте сайт Vafo.tj и снова нажмите «Войти через Telegram».',
    );
    return;
  }
...
async function handleContactShare(message: TgMessage, contact: TgContact): Promise<void> {
  ...
  if (!session) {
    await sendMessage(
      chatId,
      'Сначала нажмите «Войти через Telegram» на сайте Vafo.tj и откройте бота по ссылке.',
    );
    return;
  }
...
async function handleSubscribeStart(chatId: number, token: string): Promise<void> {
  ...
  if (!session) {
    await sendMessage(
      chatId,
      'Ссылка для подписки не найдена. Откройте сайт Vafo.tj, выберите фильтры и нажмите «Подписаться» снова.',
    );
    return;
  }
  if (new Date(session.expires_at as string) < new Date()) {
...
async function handleNonContactInput(message: TgMessage): Promise<void> {
  ...
  await sendMessage(
    chatId,
    'Я бот Vafo.tj — помогаю войти на сайт и присылаю уведомления по сохранённым квартирам. Откройте сайт Vafo.tj и нажмите «Войти через Telegram», чтобы начать.',
  );
}
