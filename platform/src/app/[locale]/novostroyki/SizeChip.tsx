...
/**
 * Apartment-size range chip ("Площадь, м²") on /novostroyki. Pairs
 * the existing PriceChip + ROOM_FILTERS pills as the apartment-level
 * filter row that gates which buildings appear in the list.
 *
 * Mirrors `kvartiry/SizeChip.tsx` (same UX + decimals + presets) but
 * lives route-local because each filter rail owns its own destination
 * path + URL-state type. Following the existing PriceChip-per-route
 * precedent — duplication is intentional and small; extracting a
 * shared block component is a separate refactor.
 *
 * Decimals allowed because Tajik listings often state half-meters
 * (45.5 m²).
 */
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1">
          <label htmlFor="nv-size-from" className="text-caption text-stone-500">
            от, м²
          </label>
          <input
            id="nv-size-from"
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="nv-size-to" className="text-caption text-stone-500">
            до, м²
          </label>
          <input
            id="nv-size-to"
